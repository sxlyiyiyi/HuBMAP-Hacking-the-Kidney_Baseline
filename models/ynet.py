...
import numpy as np
from tensorflow.keras.layers import Conv2D, Activation, Conv2DTranspose, UpSampling2D, Conv2D
...
def build_ynet(backbone, classes, skip_connection_layers,
               decoder_filters=(256, 128, 64, 32, 16),
               upsample_rates=(2, 2, 2, 2, 2),
               n_upsample_blocks=5,
    ...
    for i in range(n_upsample_blocks):
        ...
        if i < len(skip_connection_idx):
            ...
            hi = h[i]
        upsample_rate = to_tuple(upsample_rates[i])
        x1 = Upsample2(decoder_filters[i], upsample_rate=upsample_rate, use_batchnorm=use_batchnorm,
                       skip1=hi, skip2=skip_connection)(x1)

    x = Conv2D(classes, (3, 3), padding='same')(x)
    x1 = Conv2D(classes, (3, 3), padding='same', name='final_conv')(x1)
...
def ConvRelu2(filters, kernel_size, use_batchnorm=False):
    def layer(x):
...
def Upsample2(filters, kernel_size=(3, 3), upsample_rate=(2, 2),
              use_batchnorm=False, skip1=None, skip2=None):
    def layer(input_tensor):
        ...
        x = ConvRelu2(filters, kernel_size, use_batchnorm=use_batchnorm)(x)
...
if __name__ == "__main__":
    model1 = Ynet('efficientnetb2', (512, 512, 3), encoder_weights='imagenet')
    model1.summary()
